...
class Query(ObjectType):
    ...
    @staticmethod
    def _query_object(query_dict: set, django_object, **data):
        _query_dict = dict()
        for key in query_dict:
            if key in data.keys():
                _query_dict[key] = data[key]

        try:
            obj = django_object.objects.get(**_query_dict)
            return obj
        except:
            return None
    ...
    def resolve_actor(self, info, **kwargs):
        ...
        return Query._query_object(query_dict=query_set, django_object=Actor, **kwargs)

    def resolve_movie(self, info, **kwargs):
        query_set = {"id", "title"}

        return Query._query_object(query_dict=query_set, django_object=Movie, **kwargs)
